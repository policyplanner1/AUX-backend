...
function capAgeForLookup(val) {
    ...
    return Math.min(n, 81);
}

exports.calculateSuperTopupPremium = async (req, res) => {
    try {
        ...
        if (!features) return res.status(404).json({ error: "Features not found" });
        ...
        // Response
        return res.json({
            message: "ICICI Super Top-Up Premium Calculated",
            companyId,
            planId,
        ...
        console.error("Error in ICICI SuperTopup:", err);
